...
void CarList() //Hien thi danh sach xe
{
	...
	for (int i = 0; i < car.size(); i++)
	{
		...
		cout << " " << endl;
		cout << "\t|";
		cout << setfill(' ') << setw(2) << i + 1 << "| " << setfill(' ') << setw(20) << left << car[i].getName() << " | " << car[i].getFee() << "\t"
				 << " |\t" << car[i].getSeat() << "\t"
				 << "|\t" << car[i].getLeft() << "\t|"; // <<"\t";
		cout << endl;
	}
...
void invoiceInfor() //Nhap thong tin hoa don
{
	...
	if (func == 'Y' || func == 'y')
	{
		...
		Gotoxy(5, 15);
		cout << endl
				 << "\t\t\tTHANK YOU >>> SUCCESSFULLY ORDERED!!!" << endl;
		Sleep(1000);
...
void editInfor() //edit car's information
{
	...
	int seat;
	int left;
	cout << "\tName:\t" << car[id - 1].getName() << " ---> ";
	cin.ignore();
	cin.getline(name, 50);
	car[id - 1].setName(name);
	cout << "\tFee:\t" << car[id - 1].getFee() << " ---> ";
	cin >> fee;
	car[id - 1].setFee(fee);
	cout << "\tSeat:\t" << car[id - 1].getSeat() << " ---> ";
	cin >> seat;
	car[id - 1].setSeat(seat);
	cout << "\tLeft:\t" << car[id - 1].getLeft() << " ---> ";
	cin >> left;
	car[id - 1].setLeft(left);
